...
**Dependência entre tarefas** (`tarefa_bloqueadora`): a usuária pode vincular uma tarefa a outra existente como bloqueadora. A tarefa dependente fica em `aguardando` com `blocked_by_task_id` preenchido. Ao concluir a bloqueadora, `complete_task()` auto-desbloqueia todas as dependentes (status → `aberta`, campos limpos) e retorna a lista de títulos desbloqueados para notificação. O detalhe de cada tarefa exibe "⛔ Bloqueada por: X" e "🔓 Desbloqueia: Y". Tarefas bloqueadas aparecem com 🔒 nas listagens.

**Visualização `/ordem`**: comando que exibe cadeias de dependência com botões clicáveis. Cada cadeia é mostrada do objetivo final (topo, sem indentação) até a tarefa "fazer agora" (base, mais indentada). Indentação de 3 espaços por nível. Ícones: ▶ = pronta, 🔒 = bloqueada. `get_dependency_chains(chat_id)` em `task_service` constrói as cadeias com detecção de ciclos.
